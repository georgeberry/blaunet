##takes care of reading and formatting data
##accepts a rectangular data frame plus a graph object that can be coerced to the class 'network'
##should be the ONLY user callable function that is not in blaudo.r

#the main idea here is to allow the user to set up the blau object EASILY
#this function autodetects binary and continuous columns; binary columns become memberships, continuous become dimensions
#there are many possible configurations: many ecologies or one; weights or not, etc.
#there are also many different input types users are likely to use: string column names or numeric column identifiers, etc.
#this function is adapatble to these different uses and performs type checking/conversion by using helper functions:
#isCorrectLength, correctFormat, isBinary
blau <- function(square.data, graph = NULL, directed.el = FALSE, node.ids = NULL, weights = NULL, ecology.ids = NULL, exclude = NULL, dimensions = NULL, memberships = NULL, complete.cases = FALSE){
